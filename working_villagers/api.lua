...
function working_villages.register_job(job_name, def)
  ...
  -- The job-selection inventory must have at least one slot per
  -- registered job, or add_item silently drops jobs once it's full
  -- (this used to bite us once dual-skill combos pushed the total
  -- past the old fixed size of 32).
  local job_count = 0
  for _ in pairs(working_villages.registered_jobs) do
    job_count = job_count + 1
  end
  if job_count > working_villages.job_inv:get_size("main") then
    working_villages.job_inv:set_size("main", job_count + 8)
  end
